var obj = [
  ...
  function previous(j) {
    if (j != 0) {
      aprevious.setAttribute("href", "javascript:page(" + j + ");previous(" +(j-1)+ ");");
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    linum.setAttribute("class", "page-item");
    ul.append(linum);
    let anum = document.createElement("a");
    anum.setAttribute("class", "page-link");
    anum.setAttribute(
      "href",
  ...
  function next(j) {
    if (j != 11) {
      anext.setAttribute("href", "javascript:page(" + j + ");next(" +(j+1)+ ");");
    }
  }
  ...
  for (let i = 0; i < 10; i++) {
    ...
    anext.setAttribute("href","javascript:next(1);");
  }
  
  function page(j) {
    let k=10;
    for (let i = (j - 1) * k; i < (j - 1) * k + k; i++) {
  
      anext.setAttribute("href","javascript:next("+ (j+1) +");");
      aprevious.setAttribute("href","javascript:previous("+ (j-1) +");");
  ...
  document.body.append(container, ul);